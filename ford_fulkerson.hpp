...
bool bfs(const graph_t &graph, idx_t sink, idx_t target, vector<idx_t> &parent,
  ...
  while (!queue.empty()) {
    ...
    for (unsigned i = 0; i < edges.size(); ++i) {
      if (visited[edges[i].destination] || edges[i].weights[weight_idx] <= 0)
        continue;
...
double ford_fulkerson_max_flow(idx_t num_elements, const redge_c &edges,
  ...
  residual_graph.resize(num_elements);
  ...
  for (unsigned i = 0; i < edges.size(); ++i) {
    ...
    residual_graph[e.destination].edges.push_back(
        {e.source, vector<double>(e.weights.size(), 0)});
  }

  max_flow = 0;
  while (bfs(residual_graph, source, sink, parent, weight_idx)) {
...
double ford_fulkerson_max_flow(idx_t num_elements, const redge_c &edges,
                               ...
                               unsigned weight_idx = 0) {
  graph_t residual_graph;
  return ford_fulkerson_max_flow(num_elements, edges, source, sink,
                                 residual_graph, weight_idx);
}
